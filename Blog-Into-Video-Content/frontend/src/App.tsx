...
function App() {
  ...
  async function handleSubmit(ev:FormEvent) {
    ...
    const assetsResponse = await axios.get(
      API_URL+'/create-story?url='+encodeURIComponent(url)
    );
    const id = await assetsResponse.data;
    setLoadingMessage('Preparing your video...');
    const videoResponse = await axios.get(API_URL+'/build-video?id='+id);
    setLoadingMessage('');
    window.location.href = API_URL+'/'+videoResponse.data;
  }
  return (
    <>
      {loadingMessage && (
          ...
          <div className="text-gray-500 w-[240px] h-[380px] relative">
            {samples?.length > 0 && samples.map((sample,samplesKey) => (
              <video
                ...
                src={API_URL + '/' + sample + '/final.mp4'}></video>
            ))}
          </div>
        </div>
      </main>
