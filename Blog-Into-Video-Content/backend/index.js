...
async function fetchArticleContent(url) {
  ...
  if (!result || !result.text) {
    throw new Error('Exa API returned no content for this URL');
  }
  return result.text;
}
...
app.get('/create-story', async (req, res) => {
  ...
  try {
    ...
    console.error('Exa content extraction failed:', e);
    return res.status(502).json('error. could not extract content from url');
  }

  const contentFile = path + '/source-content.txt';
  fs.writeFileSync(contentFile, content);

  const opts = {
    input: `--contentFile ${contentFile} --dir ${path}`,
    disableCache: true,
  };
  try{
...
app.get('/build-video', async (req, res) => {
  ...
  if (!fs.existsSync(dir) || !fs.existsSync(dir+'/b-roll-1.png')) {
    return res.status(404).json('error. story assets not found for id: '+id);
  }
  try {
    ...
    console.error(e);
    return res.status(500).json('error renaming story assets: '+e.message);
  }
